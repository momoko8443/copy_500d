...
function bosHttpsReplace(){
	try{
		//头像
		var $headImge=$(".resume_head_div").find("img");
		if($headImge.length>0){
			var image_src=$headImge.attr("src");
			if(image_src.indexOf("http://wubaiding.bj.bcebos.com")!=-1){
				image_src=image_src.replace("http://wubaiding.bj.bcebos.com","https://wubaiding.bj.bcebos.com");
				$headImge.attr("src",image_src);
			}
		}
		//作品集
		var $workImges=$(".works_image_list").find("img");
		if($workImges.length>0){
			$workImges.each(function(index,ele){
				var image_src=$(ele).attr("src");
				if(image_src.indexOf("http://wubaiding.bj.bcebos.com")!=-1){
					image_src=image_src.replace("http://wubaiding.bj.bcebos.com","https://wubaiding.bj.bcebos.com");
					$(ele).attr("src",image_src);
				}
				
			});
		}
		//自定义图片
		var $cumstomerImges=$(".cumstomer_resume_image")
		if($cumstomerImges.length>0){
			$cumstomerImges.each(function(index,ele){
				var image_src=$(ele).attr("src");
				if(image_src.indexOf("http://wubaiding.bj.bcebos.com")!=-1){
					image_src=image_src.replace("http://wubaiding.bj.bcebos.com","https://wubaiding.bj.bcebos.com");
					$(ele).attr("src",image_src);
